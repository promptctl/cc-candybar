...
export function runDoctorCli(args: readonly string[]): never {
  ...
  const reports = runDoctor(
    gatherFacts(productionEdge(), detectTmuxHint(process.env)),
  );
      ...
      verdict.ok
        ...
        : `✗ ${check.label} — ${verdict.reason}${
            verdict.fix === undefined
              ? ""
              : ` (fix: click ☰ ${DISCLOSURE_GLYPH_CLOSED} 🧰 tools ${DISCLOSURE_GLYPH_CLOSED} 🩺 doctor ${DISCLOSURE_GLYPH_CLOSED} [fix] on the bar)`
          }\n`,
    );
  }
