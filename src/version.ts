...
// the sole authority; tsdown's `define` bakes it in at build time and Jest's
// `globals` (jest.config.js) reads the same file for untranspiled source. Every
// consumer — `--version`, the install banner, the daemon's stats snapshot —
// reads this one export; nothing else spells the version.
...
if (typeof __PACKAGE_VERSION__ === "undefined") {
  throw new Error(
    "__PACKAGE_VERSION__ was not substituted: build via tsdown (define) or run under jest (globals)",
  );
}
