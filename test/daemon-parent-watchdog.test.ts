...
// [LAW:verifiable-goals] A daemon MUST die with its spawner, teardown or not.
...
describe("pidAlive", () => {
  ...
  // [LAW:behavior-not-structure] A reused pid must not read "dead".
...
describe("daemon dies with its spawner (integration)", () => {
  ...
  afterEach(() => {
    // Signals the WHOLE group: the wrapper alone would orphan its worker.
  ...
  test("SIGKILLing the anchor process shuts the daemon down", async () => {
    ...
    // The socket appears after the watchdog is armed, so it is the ready signal.
