// [LAW:behavior-not-structure] A wire extent is sanitized to a positive int ≤ MAX or undefined.
...
describe("sanitizeTermExtent (wire trust boundary)", () => {
  ...
  test("floors non-integer positives", () => {
    ...
    expect(sanitizeTermExtent(0.9)).toBe(undefined);
...
// [LAW:behavior-not-structure] `false` and "no answer" must stay distinguishable:
// collapsing them renders a client too old to report as a confirmed-local session.
...
describe("parseClientHints (the one wire checkpoint)", () => {
  ...
  // Empty or non-string is "no override": absent, never an empty path.
  ...
  // A staged native binary does not turn over with the npm package.
