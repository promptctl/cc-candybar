...
**At 256 colours text keeps its contrast; at ansi no ratio exists** (brandon-theme-picker-bgw.fe0). The contrast floors are measured on truecolor values, and the downgrade to 256 used to break them twice over: it could land on ANSI 0–15, whose RGB every terminal theme redefines, and it rounded text and background independently, so the two roundings could meet in the middle. rich-js now downgrades only onto the cube and grey ramp (16–255), and rounds text against the background it is drawn on — the nearest entry that keeps the truecolor pair's ratio (`ColorSpec.downgradeOn`). Measured on `pnpm gallery` at 256, per text character against the truecolor render: 1171 of 3243 lost contrast and 257 landed on ANSI 0–15 before; none of either after, so every character that clears 4.5:1 in truecolor clears it at 256. `test/colour-256.test.ts` holds it. At `ansi` the terminal draws each of the sixteen colours as its own theme says, so no ratio can be computed and no floor is claimed.
