...
At ansi the one provable floor is that nothing is drawn on its own ground's index. `ensureContrast` at ansi replaces text that lands on its background's index with the nearest entry on another index; dracula's closed bar drew three characters on index 8 before. `test/colour-256.test.ts` renders every theme's closed bar and open settings states at 256 and at ansi, and holds all of this. `test/decor.test.ts` holds the state and band floors over every theme, look and depth, at both depths.
