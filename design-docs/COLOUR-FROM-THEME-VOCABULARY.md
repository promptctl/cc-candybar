...
Measured across 6 items, a monotonic ramp spreads lightness by 40 (dracula) / 50 (latte), and reads as a directional wedge — an ordering the items may or may not have. That was measured over the base-fastest vocabulary. Since brandon-theme-picker-bgw.8fp hue is the fastest axis, so evenly spaced positions step through hues and bases together, and a count whose spacing is a whole hue period lands every item on one hue: 6 items over 18 entries step by exactly 3.
...
- **Hue is the vocabulary's fastest axis.** A cell's address step moves the index only a few entries against its row's, so the fastest axis is the one neighbours most often differ in. Base-fastest spent the theme's hues on rows: a whole row was one hue, and neighbours differed by surface lightness alone (ΔE ~.04, and nothing at all on textual-ansi, whose surfaces are one colour). It is not a guarantee. A step of a whole hue period keeps the hue: van der Corput's half step at cell weight is 3.3 entries, so cells 0/1, 2/3 and 4/5 of a flat row share a hue and differ by base. The bundled bar's seams are measured, not assumed.
